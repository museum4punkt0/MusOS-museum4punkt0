'''
bluetooth beacon sensor for MusOS / Pulsar mit Erweiterung zur Integration der Station "Interaktive Holzwand". 
Der Bluetooth Beacon sensor wurde erweitert, um ankommende Besucher*innen zu registrieren und zusammen mit den Antwortversuchen 
dem Server zur Verfügung zu stellen. 
Diese werden in der Zusammenfassungsstation genutzt, um den Besucher*innen des Fasnachtsmuseum Schloss Langenstein eine Zusammenfassung des
Besuchs bieten zu können. Hier bekommen die Besucher*innen unter anderem eine Übersicht, welche Stationen besucht, welche Rätsel gelöst und welches Bild vom Maskenscanner zugeordnet wurde. 
...
# static class as temporary data model for transfer of quiz data to musOS server - first hit indicates the number of tries
...
def callback(bt_addr, rssi, packet, additional_info):
    ...
    QuizItem.beaconid = create_beacon_id(additional_info)
...
# Get beaconId and tries to correct answer on woodwall for specific user
@app.get("/woodwall/{beacon_id}")
async def getImageForId(image_id: str):
 json_quiz_item = jsonable_encoder(QuizIitem)
    return JSONResponse(content=json_quiz_item)



# Set response based on user input
@app.post("/image/{question_number}/{first_hit}")
async def setAnswer(questionNumber: int, firstHit: bool):
	QuizItem.questionnumber = questionNumber
	QuizItem.firsthit = firstHit
